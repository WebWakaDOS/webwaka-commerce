...
CREATE INDEX IF NOT EXISTS idx_review_invites_send_at ON review_invites (send_at, sent);

-- Add status column to product_reviews for moderation workflow
ALTER TABLE product_reviews ADD COLUMN status TEXT NOT NULL DEFAULT 'APPROVED';
ALTER TABLE product_reviews ADD COLUMN order_id TEXT;
ALTER TABLE product_reviews ADD COLUMN body TEXT;

CREATE INDEX IF NOT EXISTS idx_product_reviews_status ON product_reviews (tenant_id, product_id, status);
